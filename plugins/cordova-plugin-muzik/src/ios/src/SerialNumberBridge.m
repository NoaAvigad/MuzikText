...
@implementation SerialNumberBridge
...
- (void) getSerialNumber:(CDVInvokedUrlCommand *)command {
    ...
    MZSerialNumberCallback callback = ^void(NSString* serialNumber){
        ...
        if (callbackCommand != nil){
            ...
            if (serialNumber != nil){
                ...
                pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Serial number is nil."];
            }
            [self.plugin.commandDelegate sendPluginResult:pluginResult callbackId:callbackCommand.callbackId];
        }
    };
    [self.mzAccessory getSerialNumberWithBlock:callback];
}
